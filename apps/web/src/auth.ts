...
export async function signUp(input: { email: string; password: string; displayName?: string | undefined; termsVersion: string; privacyNoticeVersion: string; locale: string; termsHash: string; privacyNoticeHash: string }) {
  ...
  if (error || !data.user) throw new AppError("VALIDATION_ERROR", error?.message ?? "We could not create your account.");
}
export async function login(email: string, password: string) { const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) throw new AppError("UNAUTHORIZED", error.message); }
