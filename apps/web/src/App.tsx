...
function Dashboard() {
  ...
  return (
    <main>
      <h1>Your dogs</h1>
      {dogs.length === 0 ? (
        <EmptyState>You have no dogs yet. Add your first dog to get started.</EmptyState>
      ) : (
        <ul>{dogs.map((dog) => <li key={dog.id}>{dog.name} — {dog.profile_status.toLowerCase()}, {dog.availability_status.toLowerCase()}</li>)}</ul>
      )}
      <Discover />
      <Connections />
    </main>
